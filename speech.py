...
class SpeechMixin(object):
    ...
    def listen(self):
        ...
        for st in self.sentences():
            self.on_message(source=None, message=st)

    def sentences(self):
        while not self._stop:
            with speech.Microphone() as source:
                self.debug("Listening to microphone...")
                audio = self.recognizer.listen(source)
                self.debug("Received %i bytes of audio data." % len(audio.frame_data))

            try:
                sentence = self.recognizer.recognize_google(audio)
                self.debug("Understood: %s" % sentence)
                yield sentence

            except speech.UnknownValueError:
                self.debug("Recognizer could not understand.")
                yield ""
